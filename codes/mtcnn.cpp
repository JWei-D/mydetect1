...
Pnet::Pnet() {
    ...
    string filename = "/home/jwei/mydetec1/data/Pnet.txt";
...
Rnet::Rnet() {
    ...
    string filename = "/home/jwei/mydetec1/data/Rnet.txt";
	readData(filename, dataNumber, pointTeam);
...
Onet::Onet() {
    ...
    string filename = "/home/jwei/mydetec1/data/Onet.txt";
	readData(filename, dataNumber, pointTeam);
...
mtcnn::mtcnn() {
	nms_threshold[0] = 0.7;//0.7
	nms_threshold[1] = 0.7;//0.7
	nms_threshold[2] = 0.7;//0.7
...
void mtcnn::findFace(Mat &image, int min_size) {
	...
	for (size_t i = 0; i < scales_.size(); i++) {
		...
		proposeNet.run(reImage, scales_.at(i));
		nms(proposeNet.boundingBox_, proposeNet.bboxScore_, proposeNet.nms_threshold);

		for (vector<struct Bbox>::iterator it = proposeNet.boundingBox_.begin(); it != proposeNet.boundingBox_.end(); it++) {
			if ((*it).exist) {
		...
		proposeNet.bboxScore_.clear();
		proposeNet.boundingBox_.clear();

		proposeNet.firstFlag = true;
	...
	for (vector<struct Bbox>::iterator it = thirdBbox_.begin(); it != thirdBbox_.end(); it++) {
		if ((*it).exist) {
			rectangle(image, Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2), Scalar(0, 255, 0), 2, 8, 0);
			rectangle(image, Point((*it).y1-((*it).y2-(*it).y1), (*it).x1-0.5*((*it).x2-(*it).x1)), Point((*it).y2+((*it).y2-(*it).y1), (*it).x1+9*((*it).x2-(*it).x1)), Scalar(0, 0, 255), 2, 8, 0);
